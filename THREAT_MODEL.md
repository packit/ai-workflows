...
| Jira webhook bot-mention commands | `POST /api/jira/webhook` accepts Jira Cloud comment webhooks; a bot-mention in the comment body triggers command dispatch (e.g. `consolidate`) | Jira commenter → Redis merge-consolidation queue enqueue (gated by HMAC signature + Red Hat-employee group check on comment author) | Redis/Valkey task queues |
...
| T8 | Non-employee or compromised Jira account forces automated processing of an arbitrary issue via the `ymir_todo` label or a webhook bot-mention command (see T12 for what a legitimately-triggered issue's *content* can still drive an agent to do) | remote_auth | `ymir_todo` Jira label; Jira webhook bot-mention commands | Dist-git write access, Jira write access, compute/API budget | high | rare | mitigated | fetcher verifies label-adder is a Red Hat Employee Jira-group member via changelog walk (not JQL); atomic label flip before enqueue (fail-closed); webhook handler verifies comment author is a Red Hat Employee Jira-group member via `GET /rest/api/3/user?expand=groups` before dispatching any command (fail-closed on missing `accountId` or API error) | none |
