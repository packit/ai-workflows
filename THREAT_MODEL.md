...
|---|---|---|---|---|---|---|---|---|---|
| T1 | Unauthenticated public route grants direct read/write access to live automation task queues | remote_unauth | `redis-commander` OpenShift Route | Redis/Valkey task queues | critical | likely | unmitigated | none — route has no `HTTP_USER`/`HTTP_PASSWORD` configured; relies on hostname secrecy only | none |
...
| Verify whether `wheel`-group runtime users can `sudo` on the base images; if so, remove `wheel` membership or restrict to only the capabilities actually required | T10 | yes | S |
| Add `allowPrivilegeEscalation: false` and `capabilities: drop: [ALL]` to every long-running Deployment's securityContext, matching the existing CronJob configuration | T11 | yes | S |
