...
def get_unpacked_sources(local_clone: Path, package: str) -> Path:
    ...
    # DistGitLayout (pyrpkg) defines builddir == root_dir for dist-git repos.
    # rhpkg prep passes this as _builddir to rpmbuild.
    builddir = local_clone
    sources_dir = builddir / buildsubdir
    if sources_dir.exists():
        return sources_dir

    # RPM 4.20+ creates a per-build directory under builddir and unpacks
    ...
    async def run_workflow(
        ...
        async with mcp_tools(os.environ["MCP_GATEWAY_URL"]) as gateway_tools:
            ...
            async def fork_and_prepare_dist_git(state):
                ...
                await run_tool(
                    "download_sources", available_tools=gateway_tools,
                    dist_git_path=str(state.local_clone),
                    package=state.package,
                    dist_git_branch=state.dist_git_branch,
                )
                await run_tool(
                    "prep_sources", available_tools=gateway_tools,
                    dist_git_path=str(state.local_clone),
                    package=state.package,
                    dist_git_branch=state.dist_git_branch,
                )
                ...
                for idx, upstream_patch in enumerate(state.upstream_patches):
                    ...
                    patch_content = await run_tool(
                        "get_patch_from_url",
                        available_tools=gateway_tools,
                        patch_url=upstream_patch,
                    )
                    (state.local_clone / patch_name).write_text(patch_content)
            ...
            async def stage_changes(state):
                try:
                    # Use the spec file as the source of truth for which patches
                    # to commit.  The agent may create temporary or pre-downloaded
                    # .patch files in the working directory that should NOT be
