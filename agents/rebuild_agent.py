...
from utils import get_agent_execution_config, get_chat_model, mcp_tools, render_prompt, run_subprocess
...
async def main() -> None:
    ...
    async def run_workflow(package, dist_git_branch, jira_issue, dependency_issue=None, dependency_component=None, redis_conn=None):
        local_tool_options["working_directory"] = None
        ...
        async with mcp_tools(os.environ["MCP_GATEWAY_URL"]) as gateway_tools:
            ...
            async def commit_push_and_open_mr(state):
                try:
                    ...
                    is_empty_commit = exit_code == 0  # 0 = nothing staged

                    dep_text = f"\nDependency: {state.dependency_issue}\n" if state.dependency_issue else ""
    ...
    async with redis_client(os.environ["REDIS_URL"]) as redis:
        ...
        while True:
            ...
            try:
                logger.info(f"Starting rebuild processing for {rebuild_data.jira_issue}")
                state = await run_workflow(
                ...
                if state.rebuild_success:
                    ...
                    await fix_await(redis.lpush(
                        ...
                        ErrorData(details=f"Rebuild successful: {state.merge_request_url}", jira_issue=rebuild_data.jira_issue).model_dump_json()
                    ))
                else:
                    ...
                    await retry(task, state.rebuild_error)
