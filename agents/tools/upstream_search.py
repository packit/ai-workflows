...
class UpstreamSearchTool(Tool[UpstreamSearchToolInput, ToolRunOptions, UpstreamSearchToolOutput]):
    ...
    async def _run(
        ...
        def format_commits(repos: list[str], commits: list[str]) -> list[str]:
            # the agent struggles with finding the right format to retrieve the
            # patch from git repository hosting, so we try to resolve this for him if possible
            right_format_prefix = ""
            right_format_suffix = ""
            try:
                ...
                if parsed_url.hostname == "github.com":
                    parsed_url = parsed_url._replace(
                        path=parsed_url.path.replace(".git", "/commit/"))
                    right_format_prefix = parsed_url.geturl()
                    right_format_suffix = ".patch"
                elif parsed_url.hostname.startswith("gitlab"):
                    parsed_url = parsed_url._replace(
                        path=parsed_url.path.replace(".git", "/-/commit/"))
                    right_format_prefix = parsed_url.geturl()
                    right_format_suffix = ".patch"
            except ValueError:
                # can happen and we will fallback to letting agent guess the right format
                return commits

            return [right_format_prefix + commit + right_format_suffix for commit in commits]

        commits = format_commits(repos, commits)
