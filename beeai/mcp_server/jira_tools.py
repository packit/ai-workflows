...
async def change_jira_status(
    ...
    if os.getenv("DRY_RUN", "false").lower() == "true":
        return f"Dry run, not changing status of {issue_key} to {status}"
...
async def edit_jira_labels(
    issue_key: Annotated[str, Field(description="Jira issue key (e.g. RHEL-12345)")],
    labels_to_add: Annotated[list[str], Field(description="List of labels to add to the issue")] = None,
    labels_to_remove: Annotated[list[str], Field(description="List of labels to remove from the issue")] = None,
) -> str:
    """
    Edits labels on a Jira issue by adding and/or removing specified labels.
    """
    labels_to_add = set(labels_to_add or [])
    ...
    if os.getenv("DRY_RUN", "False").lower() == "true":
        return f"Dry run, not editing labels on {issue_key}"
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            return f"Failed to get current labels for {issue_key}: {e}"

        current_labels = set(issue_data.get("fields", {}).get("labels", []))
        updated_labels = (current_labels - set(labels_to_remove)) | set(labels_to_add)

        if updated_labels == current_labels:
            return f"No label changes needed for {issue_key}"

        try:
            async with session.put(
                jira_url,
                json={"fields": {"labels": list(updated_labels)}},
                headers=headers,
            ) as response:
                response.raise_for_status()
        except aiohttp.ClientError as e:
            return f"Failed to edit labels to {updated_labels} on {issue_key}: {e}"

    return f"Successfully edited labels on {issue_key} to {updated_labels}."
