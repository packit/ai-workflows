...
class SourcePackageCache:
    ...
    def set(self, package_name: str, source_package_name: str | None) -> None:
        ...
        if source_package_name == '':
            logging.debug(f"   Cached package not found: {package_name} → NOT_FOUND")
        else:
...
class DependencyCache:
    ...
    repeated calls for the same package.
    """
    ...
    def set(self, package_name: str, dependents: List[str]) -> None:
        """
        ...
        """
        self._cache[package_name] = dependents
        logging.debug(f"   Cached dependency results: {package_name} → {len(dependents)} dependents")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing cache statistics
        """
        total_dependents = sum(len(dependents) for dependents in self._cache.values())
        return {
            "cache_size": len(self._cache),
...
def generate_direct_dependents(
    ...
    if cache_only:
        logging.debug(f"📋 CACHE ONLY MODE: No cached dependents for {package_name}, skipping repoquery call")
        return
    ...
    dependency_cache.set(package_name, dependents_list)
...
def query_source_package(
    ...
    if cached_source_package == '' and not allow_missing:
        logging.debug(f"\n📋 SOURCE CACHE HIT: Package {package_name} → NOT_FOUND")
    ...
    for package, entry in dependents_map.items():
        ...
        while queue:
            ...
            if dependent in dependents_map:
                current_entry = dependents_map[dependent]
                if current_entry["partial"]:
                    logging.debug(f"   Propagating partial flag from {dependent} to {package}")
                    is_partial = True

                queue.extend(current_entry["dependents"])

        if filter_function is None or filter_function(package, graph.keys()):
            graph[package] = {
...
def build_dependents_graph(
    ...
    while queue:
        ...
        for dependent in generate_direct_dependents(
            ...
            if not filter_command or run_filter_command(
                dependent,
                filter_command,
                metrics,
                filter_cache,
                ...
                if package == root_package and max_results is not None:
                    ...
                    if result_count >= max_results:
                        logging.debug(
                            f"reached max_results={max_results}, no longer generating graph"
                        )
                        result_limit_hit = True
                        is_partial = True
                        break

        dependents_map[package] = {"dependents": dependents_list, "partial": is_partial or result_limit_hit}

    def filter_function(package_name: str, dependents_list: List[str]) -> bool:
        if not filter_command:
            return True

        logging.debug(f"   Filtering {package_name} with {len(dependents_list)} dependents (max_results={max_results})")
        if max_results is not None and len(dependents_list) >= max_results:
            logging.debug(f"   Reached {max_results} results, removing")
            return False

        return filter_cache.get(package_name) is not False

    dependents_graph = compute_transitive_closure(dependents_map, filter_function)
    ...
    if dependency_cache_stats["cached_packages"]:
        ...
        for package in dependency_cache_stats["cached_packages"]:
            ...
            if dependents is not None:
                print(f"     {package}: {len(dependents)} dependents", file=sys.stderr)
