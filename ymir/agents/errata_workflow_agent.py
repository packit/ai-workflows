...
async def run_errata_workflow(
    ...
    async with mcp_tools(os.getenv("MCP_GATEWAY_URL")) as gateway_tools:
        ...
        async def _erratum_comments_contain_any(erratum_id: str | int, magic_strings: list[str]) -> bool:
            """Fetch full erratum and check if any of the magic strings appear in comments."""
            ...
            return any(s in c.get("body", "") for c in comments for s in magic_strings)
        ...
        async def try_to_advance(state: ErrataWorkflowState):
            ...
            if rule_set.all_ok:
                ...
                reschedule_delay = 0 if new_status == ErrataStatus.QE else -1
                state.result = WorkflowResult(
        ...
        async def verify_product_listings(state: ErrataWorkflowState):
            ...
            for package, cur_build in cur_build_map.root.items():
                ...
                already_checked = await _erratum_comments_contain_any(
                    erratum_id,
