...
class ReasoningAgentToolTemplateDefinition(BaseModel):
    name: str
    description: str
    input_schema: str
    allowed: str
    ...
    def from_tool(cls, tool: AnyTool, *, allowed: bool = True, reason: str | None = None) -> Self:
        return cls(
            ...
            input_schema=to_json(
                tool.input_schema.model_json_schema(mode="validation"),
                indent=2,
                sort_keys=False,
            ),
            allowed=str(allowed),
...
class ReasoningAgentSystemPromptInput(BaseModel):
    role: str
...
ReasoningAgentSystemPrompt = PromptTemplate(
    schema=ReasoningAgentSystemPromptInput,
...
{{/tools.0}}
{{^tools.0}}
Tools are available via the standard tool calling mechanism.
You must use a tool to retrieve factual or historical information.
Never use the tool twice with the same input if not stated otherwise.
{{/tools.0}}
...
ReasoningAgentTaskPrompt = PromptTemplate(
    schema=ReasoningAgentTaskPromptInput,
...
class ReasoningAgentToolErrorPromptInput(BaseModel):
    reason: str


ReasoningAgentToolErrorPrompt = PromptTemplate(
    schema=ReasoningAgentToolErrorPromptInput,
...
class ReasoningAgentToolNoResultTemplateInput(BaseModel):
    tool_call: ToolInvocationResult


ReasoningAgentToolNoResultPrompt = PromptTemplate(
    schema=ReasoningAgentToolNoResultTemplateInput,
