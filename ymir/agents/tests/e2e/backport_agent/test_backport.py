...
def mock_centos_stream_repos():
    ...
    the test container and the MCP gateway can access them.  The
    ``insteadOf`` rewrites are written to ``.mock_gitconfig`` on the same
    volume by ``setup_mock_repos``; the gateway picks them up via
    ``GIT_CONFIG_GLOBAL`` and agent containers via ``include.path``.
