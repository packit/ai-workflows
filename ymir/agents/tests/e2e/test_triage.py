...
def mock_centos_stream_repos(tmp_path_factory):
    ...
    both the test container and the MCP gateway can access them.  A
    ``.mock_gitconfig`` file with ``insteadOf`` entries is written to the
    same volume; the gateway picks it up via ``GIT_CONFIG_GLOBAL`` and
    agent containers via ``include.path`` in their global gitconfig.
