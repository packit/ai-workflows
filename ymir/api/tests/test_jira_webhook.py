...
def _signed_post(client, payload: dict, secret: str = WEBHOOK_SECRET):
    """Return a coroutine that POSTs a signed webhook payload."""
    return client.post(
        "/api/jira/webhook",
        json=payload,
...
class TestVerifySignature:
    def test_valid_signature(self):
        """Known test vector from Atlassian docs."""
        raw = b"Hello World!"
        secret = "It's a Secret to Everybody"
        header = "sha256=a4771c39fbe90f317c7824e83ddef3caae9cb3d976c214ace1f2937e133263c9"
        assert _verify_signature(raw, secret, header) is True

    def test_wrong_signature(self):
        assert _verify_signature(b"payload", "secret", "sha256=0000") is False

    def test_missing_equals(self):
        assert _verify_signature(b"payload", "secret", "bad-header") is False

    def test_unsupported_method(self):
        assert _verify_signature(b"payload", "secret", "sha512=abcd") is False
...
@pytest.mark.asyncio
async def test_missing_signature_header(client):
...
@pytest.mark.asyncio
async def test_wrong_signature(client):
    resp = await client.post(
        "/api/jira/webhook",
        json=_comment_payload(_adf_mention_body("consolidate expat rhel-9.8.0")),
...
async def test_non_comment_event_ignored(client):
    payload = _comment_payload(_adf_plain_body("anything"), event="jira:issue_updated")
    resp = await _signed_post(client, payload)
