...
def get_issue_pull_requests(issue_key: str) -> list[dict[str, Any]]:
    ...
    Discovers available application types via the summary endpoint first,
    then queries each for pull requests.
    ...
    # Discover available application types via summary endpoint
    summary_url = f"{base}/rest/dev-status/1.0/issue/summary"
    summary_response = session.get(
        summary_url, headers=headers, params={"issueId": issue_id}
    )
    if not summary_response.ok:
        logger.error(
            "GET %s failed.\nerror:\n%s", summary_url, summary_response.text
        )
    raise_for_status(summary_response)

    summary_data = summary_response.json()
    app_types: list[str] = list(
    ...
    if not app_types:
        logger.debug("No application types with pull requests found for %s", issue_key)
        return []

    logger.debug("Found application types for %s: %s", issue_key, app_types)

    # Query each application type for pull requests
    pull_requests: list[dict[str, Any]] = []
    detail_url = f"{base}/rest/dev-status/1.0/issue/detail"
    for app_type in app_types:
        ...
        if not response.ok:
            logger.warning(
                "Failed to get dev-status detail for %s (applicationType=%s): %s",
                issue_key,
                app_type,
                response.text,
            )
