...
async def search_resultsdb(package_nvr: str, name_pattern: str) -> list[ResultsDbResult]:
    ...
    async with (
        aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT, headers={"User-Agent": YMIR_USER_AGENT}) as session,
...
class SearchResultsdbTool(Tool[SearchResultsdbInput, ToolRunOptions, SearchResultsdbOutput]):
    ...
    description = (
        ...
        "test results will be in resultsdb."
    )
    input_schema = SearchResultsdbInput
