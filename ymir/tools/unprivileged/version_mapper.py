...
class VersionMapperResult(BaseModel):
    y_stream: str | None = Field(
        description="Current Y-stream version for the major version (None if no Y-stream available)"
    )
    z_stream: str | None = Field(
        description="Current Z-stream version for the major version (None if no Z-stream available)"
    )
    is_maintenance_version: bool = Field(
        description="True if this is a maintenance version (no Y-stream available)"
    )
